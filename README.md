...
```
pip install mujoco whitetree
python -m flyswing.course --episodes 20
```

This runs 20 random courses and prints the score of each. The extracted escape circuit ships in `data/malecns`. To run the whole-connectome network, `pip install pyarrow pandas` and run `python -m flyswing.circuit.wholecns`. It downloads the 1.1 GB MaleCNS weights file on first use.
